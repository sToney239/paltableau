#' Diverging gradient fill scales with Tableau palettes.
#'
#' @param palette Palette from "paltableau" package. <br> Try to type "pal_tableau$" and wait for hint of "regular", "sequential" or "diverging",<br> select one, and type "$" to choose the ultimate palette.
#' @param ... Other arguments passed on to \code{\link[ggplot2]{binned_scale}}.
#' @param na.value Colour to use for missing values
#' @param direction Sets the order of colors in the scale. If 1, the default, colors are ordered from darkest to lightest. If -1, the order of colors is reversed.
#' @param start The (corrected) color range in \[0,1\]⁠ at which the color map starts.
#' @param end The (corrected) color range in \[0,1\]⁠ at which the color map ends.
#' @param midpoint The midpoint (in data value) of the diverging scale.<br>Defaults to 0.
#' @param breaks The breaks for bins
#' @param transform For continuous scales, the name of a transformation object or the object itself. Built-in transformations include "asn", "atanh", "boxcox", "date", "exp", "hms", "identity", "log", "log10", "log1p", "log2", "logit", "modulus", "probability", "probit", "pseudo_log", "reciprocal", "reverse", "sqrt" and "time".<br>A transformation object bundles together a transform, its inverse, and methods for generating breaks and labels. Transformation objects are defined in the scales package, and are called ⁠`transform_<name>`⁠. If transformations require arguments, you can call them from the scales package, e.g \code{\link[scales]{transform_boxcox}}... You can create your own transformation with \code{\link[scales]{new_transform}}.
#' @param guide Type of legend. Could be "colorbar", "colorsteps", "legend"
#' @export
#' @examples library(ggplot2)
#' ggplot(iris)+
#'   geom_col(aes(Sepal.Length, Sepal.Width, fill = Sepal.Length))+
#'   scale_fill_tableau_steps2(palette = pal_tableau$diverging$temperature,
#'                             guide = guide_bins(
#'                             theme = theme(legend.ticks = element_blank(),legend.axis.line = element_line(color = "transparent"))))

scale_fill_tableau_steps2 = function(palette = pal_tableau$diverging$red_blue_white,
                                     na.value = "grey50",
                                     direction = 1,start = 0, end = 1,midpoint=0,
                                     breaks = NULL,
                                     guide = "colorsteps",
                                     transform = "identity",...) {
  manual_rescale2 = rescale_gradient2(palette,direction,start,end)

  if (!is.null(breaks)) {
    manual_rescale = \(x) manual_rescale2((seq_len(length(breaks)-1)-1)/(length(breaks)-2))
  } else {
    breaks = ggplot2::waiver()
    manual_rescale = manual_rescale2
  }

  ggplot2::binned_scale("fill",
                        palette=  manual_rescale,
                        na.value = na.value,
                        transform = transform,
                        show.limits = TRUE,
                        breaks = breaks,
                        guide = guide,
                        rescaler = mid_rescaler(mid = midpoint, transform = transform),
                        ...,
                        oob = scales::squish)
}


#' Tableau color scales (binned & low-mid-high)
#'
#' @param palette Palette from "paltableau" package. <br> Try to type "pal_tableau$" and wait for hint of "regular", "sequential" or "diverging",<br> select one, and type "$" to choose the ultimate palette.
#' @param ... Other arguments passed on to \code{\link[ggplot2]{binned_scale}}.
#' @param na.value Colour to use for missing values
#' @param direction Sets the order of colors in the scale. If 1, the default, colors are ordered from darkest to lightest. If -1, the order of colors is reversed.
#' @param start The (corrected) color range in \[0,1\]⁠ at which the color map starts.
#' @param end The (corrected) color range in \[0,1\]⁠ at which the color map ends.
#' @param midpoint The midpoint (in data value) of the diverging scale.<br>Defaults to 0.
#' @param breaks The breaks for bins
#' @param guide Type of legend. Could be "colorbar", "colorsteps", "legend"
#' @param transform For continuous scales, the name of a transformation object or the object itself. Built-in transformations include "asn", "atanh", "boxcox", "date", "exp", "hms", "identity", "log", "log10", "log1p", "log2", "logit", "modulus", "probability", "probit", "pseudo_log", "reciprocal", "reverse", "sqrt" and "time".<br>A transformation object bundles together a transform, its inverse, and methods for generating breaks and labels. Transformation objects are defined in the scales package, and are called ⁠`transform_<name>`⁠. If transformations require arguments, you can call them from the scales package, e.g \code{\link[scales]{transform_boxcox}}... You can create your own transformation with \code{\link[scales]{new_transform}}.
#'
#' @export
#' @examples library(ggplot2)
#' library(paltableau)
#' ggplot(iris)+
#'   geom_point(aes(Sepal.Length, Sepal.Width, color = Sepal.Length))+
#'   scale_color_tableau_steps2(palette = pal_tableau$diverging$red_blue_white, direction = 1,
#'                              guide = guide_bins(theme = theme(legend.ticks = element_blank(),legend.axis.line = element_line(color = "transparent"))))

scale_color_tableau_steps2 = function(palette = pal_tableau$diverging$red_blue_white,
                                      na.value = "grey50",
                                      direction=1,start = 0, end = 1,midpoint=0,
                                      breaks = NULL,
                                      guide = "coloursteps",
                                      ...,
                                      transform = "identity") {
  manual_rescale2 = rescale_gradient2(palette,direction,start,end)

  if (!is.null(breaks)) {
    manual_rescale = \(x) manual_rescale2((seq_len(length(breaks)-1)-1)/(length(breaks)-2))
  } else {
    breaks = ggplot2::waiver()
    manual_rescale = manual_rescale2
  }

  ggplot2::binned_scale("color",
                        palette=  manual_rescale,
                        na.value = na.value,
                        transform = transform,
                        show.limits = TRUE,
                        breaks = breaks,
                        guide = guide,
                        rescaler = mid_rescaler(mid = midpoint, transform = transform), # ggplot2:::mid_rescaler(mid = midpoint, transform = transform),
                        ...,
                        oob = scales::squish)

}




#' Diverging gradient fill scales with Tableau palettes.
#' @description
#' `r lifecycle::badge("deprecated")`\cr
#' Please use \code{\link[paltableau]{scale_fill_tableau_steps2}}.
#'
#' @param palette Palette from "paltableau" package. <br> Try to type "pal_tableau$" and wait for hint of "regular", "sequential" or "diverging",<br> select one, and type "$" to choose the ultimate palette.
#' @param ... Other arguments passed on to \code{\link[ggplot2]{binned_scale}}.
#' @param na.value Colour to use for missing values
#' @param direction Sets the order of colors in the scale. If 1, the default, colors are ordered from darkest to lightest. If -1, the order of colors is reversed.
#' @param start The (corrected) color range in \[0,1\]⁠ at which the color map starts.
#' @param end The (corrected) color range in \[0,1\]⁠ at which the color map ends.
#' @param midpoint The midpoint (in data value) of the diverging scale.<br>Defaults to 0.
#' @param breaks The breaks for bins
#' @param transform For continuous scales, the name of a transformation object or the object itself. Built-in transformations include "asn", "atanh", "boxcox", "date", "exp", "hms", "identity", "log", "log10", "log1p", "log2", "logit", "modulus", "probability", "probit", "pseudo_log", "reciprocal", "reverse", "sqrt" and "time".<br>A transformation object bundles together a transform, its inverse, and methods for generating breaks and labels. Transformation objects are defined in the scales package, and are called ⁠`transform_<name>`⁠. If transformations require arguments, you can call them from the scales package, e.g \code{\link[scales]{transform_boxcox}}... You can create your own transformation with \code{\link[scales]{new_transform}}.
#' @param guide Type of legend. Could be "colorbar", "colorsteps", "legend"
scale_fill_tableau_binned_gradient2 = function(palette = pal_tableau$diverging$red_blue_white,
                                               na.value = "grey50",
                                               direction = 1,start = 0, end = 1,midpoint=0,
                                               breaks = NULL,
                                               guide = "colorsteps",
                                               transform = "identity",...)  {
  lifecycle::deprecate_warn("0.1.0", "scale_color_tableau_binned_gradient2()", "scale_fill_tableau_steps2()")
  scale_fill_tableau_steps2(palette = pal_tableau$diverging$red_blue_white,
                            na.value = "grey50",
                            direction = 1,start = 0, end = 1,midpoint=0,
                            breaks = NULL,
                            guide = "colorsteps",
                            transform = "identity",...)
}


#' Tableau color scales (binned & low-mid-high)
#' @description
#' `r lifecycle::badge("deprecated")`\cr
#' Please use \code{\link[paltableau]{scale_color_tableau_steps2}}.
#'
#' @param palette Palette from "paltableau" package. <br> Try to type "pal_tableau$" and wait for hint of "regular", "sequential" or "diverging",<br> select one, and type "$" to choose the ultimate palette.
#' @param ... Other arguments passed on to \code{\link[ggplot2]{binned_scale}}.
#' @param na.value Colour to use for missing values
#' @param direction Sets the order of colors in the scale. If 1, the default, colors are ordered from darkest to lightest. If -1, the order of colors is reversed.
#' @param start The (corrected) color range in \[0,1\]⁠ at which the color map starts.
#' @param end The (corrected) color range in \[0,1\]⁠ at which the color map ends.
#' @param midpoint The midpoint (in data value) of the diverging scale.<br>Defaults to 0.
#' @param breaks The breaks for bins
#' @param guide Type of legend. Could be "colorbar", "colorsteps", "legend"
#' @param transform For continuous scales, the name of a transformation object or the object itself. Built-in transformations include "asn", "atanh", "boxcox", "date", "exp", "hms", "identity", "log", "log10", "log1p", "log2", "logit", "modulus", "probability", "probit", "pseudo_log", "reciprocal", "reverse", "sqrt" and "time".<br>A transformation object bundles together a transform, its inverse, and methods for generating breaks and labels. Transformation objects are defined in the scales package, and are called ⁠`transform_<name>`⁠. If transformations require arguments, you can call them from the scales package, e.g \code{\link[scales]{transform_boxcox}}... You can create your own transformation with \code{\link[scales]{new_transform}}
scale_color_tableau_binned_gradient2 = function(palette = pal_tableau$diverging$red_blue_white,
                                                na.value = "grey50",
                                                direction = 1,start = 0, end = 1,midpoint=0,
                                                breaks = NULL,
                                                guide = "colorsteps",
                                                transform = "identity",...)  {
  lifecycle::deprecate_warn("0.1.0", "scale_color_tableau_binned_gradient2()", "scale_color_tableau_steps2()")
  scale_color_tableau_steps2(palette = pal_tableau$diverging$red_blue_white,
                             na.value = "grey50",
                             direction = 1,start = 0, end = 1,midpoint=0,
                             breaks = NULL,
                             guide = "colorsteps",
                             transform = "identity",...)
}
